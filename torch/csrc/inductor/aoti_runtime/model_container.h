...
class AOTInductorModelContainer {
 ...
 public:
  ...
  void set_use_stream_affinity(bool use_stream_affinity) {
    ...
    if (available_models_.size() != models_.size() ||
        !pending_models_.empty() || !checked_out_models_.empty()) {
  ...
  int64_t get_stream_affinity_model_index(DeviceStreamType stream) {
    ...
    throw std::runtime_error("Stream affinity refers to an unknown model");
  }
