...
class UserMethodVariable(UserFunctionVariable):
    ...
    def call_function(
        ...
        if (
            ...
            # generic-construction gate). This mirrors the same symbolic
            # (non-eager) reconstruction UserDefinedObjectVariable.call_function
            # already uses for a bound `clone` reached as a plain callable
            # value, scoped identically: only the base
            # _DecoratorContextManager.clone (no_grad, enable_grad) and the
            # dedicated inference_mode.clone case, both of which have no
            # side effects when the resulting instance is discarded. Other
            # overrides (e.g. set_grad_enabled.clone, which mutates real
            # global autograd state as a side effect of construction) fall
            # through to the normal inlining path below and keep graph-
            # breaking until a source is available.
            from .torch import TorchCtxManagerClassVariable

            cm_obj = self.obj.value
            cm_cls = type(cm_obj)
            if (
            ...
            if (
                self.fn is torch.autograd.grad_mode.inference_mode.clone
                and cm_cls is torch.autograd.grad_mode.inference_mode
            ):
                mode_var = VariableTracker.build(tx, cm_obj.mode)  # type: ignore[attr-defined]
