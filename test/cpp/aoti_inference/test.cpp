...
void test_cuda_stream_affinity_bindings(const std::string& device) {
  ...
  EXPECT_EQ(
      ...
      default_loader.get_runner()->get_stream_affinity_model_index(
          first_stream.stream()));
  ...
  EXPECT_EQ(
      ...
      affinity_loader.get_runner()->get_stream_affinity_model_index(
          first_stream.stream()));
  ...
  const auto only_model =
      oversubscribed_loader.get_runner()->get_stream_affinity_model_index(
          first_stream.stream());
...
void test_concurrent_same_cuda_stream_affinity(const std::string& device) {
  ...
  EXPECT_GE(
      loader.get_runner()->get_stream_affinity_model_index(stream.stream()), 0);
}
...
void test_aoti_const_fold_separate_stream(
  ...
  for (int i = 0; i < num_iters; ++i) {
    ...
    if (use_stream_affinity) {
      ASSERT_GE(
          runner->get_stream_affinity_model_index(fold_stream.stream()), 0);
    }
