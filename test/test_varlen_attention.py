...
class _VarlenVsSdpaMixin:
    # Shared by TestVarlenAttention and TestVarlenAttentionCuDNN, which cannot
    # inherit from each other: their tests target different backends.
...
class TestVarlenAttentionDevice(NNTestCase):
    # Scale validation and forced-backend rejection happen before any kernel is
    # dispatched, so these checks hold on every device.
...
class TestVarlenAttention(_VarlenVsSdpaMixin, NNTestCase):
    # NOTE: These tests exercise the flash backends, which every accelerator can
    # in principle provide, but they stay CUDA-specific for now: the backend
    # matrix is spelled in terms of FA2/FA3/FA4 and SM capabilities, and only
    # CUDA has a varlen flash kernel behind aten::_flash_attention_forward.
    # Reclassifying this as ACCELERATOR requires a Capability mechanism through
    # which each backend reports the flash implementations it supports.
    # test_varlen_lse_is_not_differentiable and test_batch_invariance also
    # parametrize over cuDNN: they compare flash against cuDNN rather than being
    # cuDNN-specific, so they stay here while both classes are CUDA-only. Their
    # cuDNN parameter has to be split off before this class can go ACCELERATOR.
