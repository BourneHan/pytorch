...
class TestGpuWrapper(InductorTestCase):
    ...
    @skipIfXpu(msg="tests CUDA TMA helper codegen")
    def test_tma_descriptor_caches_cuda_driver_version(self):
        if torch.version.hip is not None:
            self.skipTest("requires CUDA TMA helpers")

        helpers = CUDADeviceOpOverrides().tma_descriptor_helpers()
        self.assertIn("#include <mutex>", helpers)
        self.assertIn("static std::once_flag driverVersionInitFlag;", helpers)
        self.assertIn("std::call_once(driverVersionInitFlag, []() {", helpers)
        self.assertIn("CUDA_DRIVER_CHECK(cuDriverGetVersion(&driverVersion));", helpers)
        self.assertEqual(helpers.count("cuDriverGetVersion(&driverVersion)"), 1)
